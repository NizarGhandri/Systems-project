...
object Utils {
    ...
    def compute_similarity_u(user: Int, pr: Broadcast[CSCMatrix[Double]], k: Int) = {
        val preprocessed = pr.value
        val users = preprocessed.rows
        val items = preprocessed.cols
        val builder = new CSCMatrix.Builder[Double](rows=users, cols=users)
        val all_sims = preprocessed * preprocessed(user, 0 until items).t.toDenseVector
        for (j <- argtopk(all_sims, k+1)){
            if (j != user) builder.add(user, j, all_sims(j))
        }
        builder.result()
    }

    def compute_similarity(x: RDD[Int], preprocessed: Broadcast[CSCMatrix[Double]], k: Int) = {
        x.map(u => compute_similarity_u(u, preprocessed, k)).reduce(_+_)
    }

    def global_dev_i(item: Int, dev:  Broadcast[CSCMatrix[Double]], sim: Broadcast[CSCMatrix[Double]]) = {
        val similarities = sim.value
        val deviations = dev.value
        ...
        val vect = deviations(0 until r, item).toDenseVector 
        val adj = (similarities * vect) /:/  (l1 * vect.map(x => if(x == 0.0) 0.0 else 1.0))
        for (k <- 0 until r) {
            val v = adj(k)
            if (!v.isNaN) builder.add(k, item, v)   
        }
        builder.result()
    }

    def global_dev(x: RDD[Int], deviations: Broadcast[CSCMatrix[Double]], similarities: Broadcast[CSCMatrix[Double]]) = {
        x.map(i => global_dev_i(i, deviations, similarities)).reduce(_+_)
    }
    ...
    def sim_dev_apu (train: CSCMatrix[Double], k: Int, sc: SparkContext) = {
        val (average_per_user, deviations, preprocessed_deviations) = preprocess_similarity(train)
        val br = sc.broadcast(preprocessed_deviations)
        val similarities = compute_similarity(sc.parallelize((0 until train.rows)), br, k)
        (similarities, deviations, average_per_user)
    }

    def knn(test: CSCMatrix[Double], train: CSCMatrix[Double], k: Int, global_average: Double, sc: SparkContext) = {
        val (similarities, deviations, average_per_user) = sim_dev_apu(train, k, sc)
        val br_sim = sc.broadcast(similarities)
        val br_dev = sc.broadcast(deviations)
        predict(test, average_per_user, global_dev(sc.parallelize((0 until train.cols)), br_dev, br_sim), global_average)
    }
